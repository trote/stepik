...
sub add {
    ...
    unless ( grep { $_ eq $var } @{$vars{$namespace}} ) {
        push @{$vars{$namespace}}, $var;
    }
}

sub create {
    my ( $namespace, $parent ) = @_;

    unless ( $namespaces{$parent} ) {
        $namespaces{$parent} = [];
        $vars{$parent} = [];
    }
    ...
    unless ( $namespaces{$namespace} ) {
        ...
        $vars{$namespace} = [];
    } elsif ( ! grep { $_ eq $parent } @{$namespaces{$namespace}} ) {
        push @{$namespaces{$namespace}}, $parent;
    }
}
...
sub get {
    ...
    my $result = 'None';
    if ( grep { $_ eq $var } @{$vars{$namespace}} ) {
       $result = $namespace;
    } else {
        for my $parent ( @{$namespaces{$namespace}} ) {
            $result = get($parent, $var);
            last if $result ne 'None';
        }
    }
    return $result;
}
